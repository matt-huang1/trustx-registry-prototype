...
| [0019](adr/0019-per-type-profiles-aligned-to-current-all-12-rule.md) | Per-type profiles aligned to the current all-12 rule following RAI confirmation that the v1.1 subset formulas are outdated; legacy profiles retained as auditable history (KYC medium → high, summarisation low → medium; RAI seed tiers unchanged) | Accepted |
...
- Attribution (no ADR — provenance, not a design decision): the ARC framework's source is
  ...
  by ADR-0017, and the divergence itself was resolved by ADR-0019 after RAI confirmed
  (2026-08-16) that the v1.1 per-type subset formulas are outdated.
