...
*Worst-case-wins, and weighting as explicit policy.* The overall tier is derived by a worst-case-wins rule (the paper's "critical dimension" approach): any single dimension at the top of the scale makes the whole system high-risk, and a dimension's severity is never averaged away by the calmer dimensions around it. This is deliberate and it matters, because averaging is exactly how a genuinely dangerous capability gets hidden behind a system's otherwise modest profile. Adopting RAI's model surfaced a subtlety worth stating plainly: the published paper specifies this rule across all twelve dimensions (Liu, Saxena, and Asthana, arXiv:2607.09586, Section 3.3), while the v1.1 per-type classification tools tiered off different subsets of them. Rather than silently pick one interpretation, this design separates two things that had been conflated. Every agent is always scored on all twelve dimensions, so the full picture is always recorded. Which dimensions drive the tier is then a named, per-type weighting profile — data, not a formula buried in a spreadsheet. RAI has since confirmed that the subset formulas were outdated and the paper's rule is the current one, so every profile now weights all twelve; the superseded subsets are kept as labelled history rather than deleted ([ADR-0019](adr/0019-per-type-profiles-aligned-to-current-all-12-rule.md)). That correction is the best evidence for the split: it re-tiered two entries as a one-line data change, with nothing re-scored, no rollup code touched, and each affected entry recording which dimension moved it. The tiering rule is something a governance body can see, audit, and set deliberately, instead of an implicit choice inherited from whichever tool was used.
...
The repository is structured to connect to the RAI-Institute organisation without rework when the working group chooses to adopt it. The path is: transfer or fork the repository into the organisation; the contribution pipeline continues unchanged, since entries already arrive as pull requests gated by the automated checks; and the governance model's 'identified maintainers' become the organisation's own working-group members, which is what GOVERNANCE.md already assumes. Nothing in the registry binds it to a personal account — the licences, the citation of the ARC paper as the framework's source, and the trust-tier machinery are all already written for institutional ownership.
...
*Who owns the tier weighting, and how is a change to it ratified?* Building against the ARC model surfaced a divergence: the published paper specifies the critical-dimension rule across all twelve dimensions — worst-case-wins, any dimension at its highest level yielding Tier 3 (Liu, Saxena, and Asthana, arXiv:2607.09586, Section 3.3) — while the v1.1 per-type classification tools tier off subsets of them. The paper's own Section 5.2 Decision Support example, which reaches Tier 3 on Data Sensitivity alone against an average of 1.42, confirmed it. **That much is now settled.** RAI confirmed on 2026-08-16, from the framework's author, that the subset formulas are an outdated version of the model and Section 3.3 is the current rule; the divergence was an implementation artefact of the tooling, not a second defensible reading. Every per-type profile has been aligned accordingly, with the superseded subsets retained as labelled history ([ADR-0019](adr/0019-per-type-profiles-aligned-to-current-all-12-rule.md)).

What that resolution does not settle is the governance question underneath it, and the migration made it concrete. Two committed entries re-tiered upward on a one-line data change arriving as a verbal confirmation, with no ratification step, no notification to anyone relying on the old tiers, and no versioned record of the entries as they stood the day before. This design separates scoring from weighting precisely so the choice is explicit, auditable data rather than an implicit consequence of which tool was used — which is what made the correction cheap and every re-tier attributable. But cheap to change is not the same as safe to change. Who owns the weighting, what evidence a change to it must carry, who ratifies it, and what the registry owes the people relying on an entry whose tier is about to move are working-group decisions, not engineering ones. A change to the weighting re-tiers entries across the registry; it should not be able to happen quietly.
