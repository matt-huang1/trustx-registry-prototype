...
*The contribution pipeline.* The registry is a git repository of structured entries, one file per agent, and a contribution is a pull request. Before a maintainer sees it, automated checks run against it. The entry must conform to the schema. Every dimension must carry a rationale and evidence drawn from the agent's own description; a score with nothing behind it fails. The deterministic rules apply, so a submission that describes a money-moving agent cannot enter with a comfortable Action Authority score, whatever the submitter claims. The tier is recomputed from the scores, so a declared tier that flatters the evidence fails the build rather than quietly landing in the registry. And the challenger runs as an automated first-pass reviewer, red-teaming the submission and flagging any score the description does not support. Only what survives all of that reaches a person. The effect is that the cost of a careless or dishonest submission is paid by the submitter, not by the maintainer.
