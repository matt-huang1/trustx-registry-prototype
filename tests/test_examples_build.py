...
def test_examples_exist_and_are_schema_valid():
    ...
    for ex in examples:
        ...
        assert ex["risk_tier"] == roll_up_risk_tier(ex["dimensions"])
...
def test_money_movement_example_shows_the_deterministic_floor():
    ...
    ex = high[0]
    da = ex["dimensions"]["delegated_authority"]
    assert da["score"] >= MONEY_MOVEMENT_FLOOR
    # The override must be visible in the audit trail (evidence marker + challenge note).
    assert any("deterministic-rule" in e for e in da["evidence"])
